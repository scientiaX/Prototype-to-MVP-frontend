# 📚 NovaX Documentation

This folder contains all project documentation. For easy navigation:

## 🚀 Getting Started
- [QUICKSTART.md](./QUICKSTART.md) - Quick start guide
- [README-SETUP.md](./README-SETUP.md) - Detailed setup instructions

## 🐳 Deployment
- [DOCKER-GUIDE.md](./DOCKER-GUIDE.md) - Docker setup and usage
- [RAILWAY-DEPLOY-GUIDE.md](./RAILWAY-DEPLOY-GUIDE.md) - Railway deployment
- [README-DEPLOYMENT.md](./README-DEPLOYMENT.md) - General deployment guide
- [ENV-GUIDE.md](./ENV-GUIDE.md) - Environment variables reference

## 🎮 Arena System
- [AI_ARENA_SYSTEM.md](./AI_ARENA_SYSTEM.md) - AI Arena system overview
- [arena_engine_specification_interaction_ai_decision_layer.md](./arena_engine_specification_interaction_ai_decision_layer.md) - Arena engine specs
- [arena_experience_layer_intuitive_high_impact_ux.md](./arena_experience_layer_intuitive_high_impact_ux.md) - UX layer design
- [arena_first_3_minutes_high_impact_entry_design.md](./arena_first_3_minutes_high_impact_entry_design.md) - Entry flow design

## 🎯 NovaX Design
- [nova_x_desain_mode_10_menit.md](./nova_x_desain_mode_10_menit.md) - 5-minute mode design
- [nova_x_onboarding_arena_pbl_first_match_tutorial.md](./nova_x_onboarding_arena_pbl_first_match_tutorial.md) - Onboarding PBL specs
- [nova_x_missing_execution_specs_thread_audit.md](./nova_x_missing_execution_specs_thread_audit.md) - Execution specs audit
- [penghilangan_friksi_kognitif_inti_nova_x_draft_eksekusi.md](./penghilangan_friksi_kognitif_inti_nova_x_draft_eksekusi.md) - Cognitive friction reduction
